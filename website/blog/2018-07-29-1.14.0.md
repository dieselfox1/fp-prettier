...
> Note: the [`proseWrap`](https://fintasticfish.us/docs/en/options.html#prose-wrap) option is set to `preserve` by default so you’ll have to specify `always` or `never` to enable this feature.
...
[`--require-pragma`](https://fintasticfish.us/docs/en/options.html#require-pragma) and
[`--insert-pragma`](https://fintasticfish.us/docs/en/options.html#insert-pragma).
...
as a workaround for indented codeblock mis-parsing](https://fintasticfish.us/blog/2018/02/26/1.11.0.html#respect-tabwidth-for-list-items-3990-by-ikatyang),
...
but our [range format](https://fintasticfish.us/docs/en/options.html#range) option didn’t quite work properly when selecting the contents of a function.
...
Previously, [range formatting](https://fintasticfish.us/docs/en/options.html#range) was not available in Vue,
